...
std::list<std::string> ZCbpControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCbpControlDialog::listMints;

ZCbpControlDialog::ZCbpControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCbpControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCbpControlDialog::~ZCbpControlDialog()
{
    delete ui;
}

void ZCbpControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCbpControlDialog::updateList()
{
...
void ZCbpControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCbpControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCbp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCbpControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCbpControlDialog::GetSelectedMints()
{
...
void ZCbpControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
