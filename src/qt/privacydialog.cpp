                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCBP ought to be enough for anybody." - Bill Gates, 2017
    ui->zCBPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCBPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCBP->setEnabled(false);
        ui->pushButtonMintzCBP->setToolTip(tr("zCBP is currently disabled due to maintenance."));

        ui->pushButtonSpendzCBP->setEnabled(false);
        ui->pushButtonSpendzCBP->setToolTip(tr("zCBP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCBPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCBP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCBP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCBP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCBP in ") +
...
void PrivacyDialog::on_pushButtonSpendzCBP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCBP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCBP
        sendzCBP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCBP
    sendzCBP();
}

void PrivacyDialog::on_pushButtonZCbpControl_clicked()
{
    ZCbpControlDialog* zCbpControl = new ZCbpControlDialog(this);
    zCbpControl->setModel(walletModel);
    zCbpControl->exec();
}

void PrivacyDialog::setZCbpControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCbpSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCBP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cbp Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCBPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCBP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CBP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCBP</b>";
    ...
    // use mints from zCbp selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCbpControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCbpControlDialog::GetSelectedMints();
    }
    ...
    ZCbpControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCbp Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCBP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCBP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCBP "));
    ui->labelzCBPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCBPSyncStatus->setVisible(fShow);
}
