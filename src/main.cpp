...
#if defined(NDEBUG)
#error "Cbp cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight >= 2 && nHeight <= 2500) {
      nSubsidy = 20 * COIN;
    } else if (nHeight >= 2501 && nHeight <= 32000) {
      nSubsidy = 40 * COIN;
    } else if (nHeight >= 32001 && nHeight <= 102000) {
      nSubsidy = 80 * COIN;
    } else if (nHeight >= 102001 && nHeight <= 202000) {
      nSubsidy = 60 * COIN;
    } else if (nHeight >= 202001 && nHeight <= 412000) {
      nSubsidy = 50 * COIN;
    } else if (nHeight >= 412001 && nHeight <= 822000) {
      nSubsidy = 30 * COIN;
    } else if (nHeight >= 822001) {
      nSubsidy = 20 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Cbp
...
{
    RenameThread("cbp-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCBPMinted()
{
...
void RecalculateZCBPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zCBP supply
...
bool RecalculateCBPSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CBP to zCBP
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Cbp: We use certain sporks during IBD, so check to see if they are
