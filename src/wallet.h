...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CBP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCBP send
enum ZerocoinSpendStatus {
    ZCBP_SPEND_OKAY = 0,                            // No error
    ZCBP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCBP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCBP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCBP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCBP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCBP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCBP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCBP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCBP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCBP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCBP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCBP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCBP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCBP_SPENT_USED_ZCBP = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZJewAutoBackups(bool fEnabled)
    {
